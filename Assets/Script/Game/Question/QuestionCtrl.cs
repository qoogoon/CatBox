...
public class QuestionCtrl : MonoBehaviour {
    ...
    public void createQuestion()
    {
        ...
        //화면 표기 호출
        arrRenderQuestion = questionRespond.renderQuestion(arrLogicQuestion);
        ...
        if (bQuestion && !Constant.gameCtrl.getGameOverState() && !bBtnUsing && arrLogicQuestion.Count > 0 && bBtnEnble)
        {
            ...
            if ((int)arrLogicQuestion[nBtnTouchCounter]%4 == nBtnNo || Constant.comboCtrl.isFever())
            {
                ...
                if (arrLogicQuestion.Count <= nBtnTouchCounter)   //총정답
                {
                    ...
                    //첫번째 고양이 박스 고양이 상태로 변경
                    GameObject firstCatObj = (GameObject)Constant.catCtrl.getCatObjs()[0];
                    firstCatObj.GetComponent<CatObj>().BoxCat = true;

                    //고양이 큐 조정
                    clearQuestion();
                    Constant.catCtrl.setSuccessCat(firstCatObj);   //선두 고양이를 성공 고양이로 설정
                    Constant.catCtrl.getCatObjs().RemoveAt(0);  //고양이 큐에서 선두 고양이 삭제
